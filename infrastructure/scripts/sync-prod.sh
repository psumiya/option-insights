...
# Quick sync script for PRODUCTION environment
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Quick Sync - PRODUCTION Environment${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
echo -e "${YELLOW}⚠️  WARNING: You are about to sync to PRODUCTION${NC}"
echo -e "${YELLOW}   This will update the live website at: ${SUBDOMAIN}.${DOMAIN_NAME}${NC}"
echo ""
...
if ! aws cloudformation describe-stacks \
    ...
    echo -e "${YELLOW}Run the full deployment script first: ./deploy-production.sh${NC}"
    exit 1
fi
